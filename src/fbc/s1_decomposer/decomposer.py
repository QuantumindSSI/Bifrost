...
class MambaBlock(nn.Module):
    """SSM block: real mamba-ssm on CUDA, S6SelectiveScan everywhere else.
    ...
    def __init__(
        ...
        if self.use_cuda_mamba:
            ...
            self.ssm = S6SelectiveScan(
                d_model=d_model, d_state=d_state, d_conv=d_conv, expand=expand
            )
            self.ssm_type = "S6SelectiveScan (PyTorch)"
...
class S1SpectralDecomposer(nn.Module):
    ...
    def ssm_type(self) -> str:
        ...
        return "S6SelectiveScan (PyTorch)"
